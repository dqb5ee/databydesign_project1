...
# 1. LOAD RAW DATA
# Using the specific perimeter files you uploaded
...
# 2. PROCESS FIRE PERIMETERS
# Merge the two parts and remove duplicates
fires = pd.concat([f1, f2]).drop_duplicates(subset=['OBJECTID'])
fires['Alarm Date'] = pd.to_datetime(fires['Alarm Date'], errors='coerce')

# Filter for Southern California Units and the 2005-2019 study window
